...
.net standard 2.0 是最後一個同時支援 core 跟 f 的 
...
(第一天下午 32:33 )
版本的長短是一種迷思，每個版本本來就會包含上一個版本的所有內容
直接升上去好處多多，比壞處多很多
...
```sh
PS C:\Users\h1273> dotnet nuget locals all -c
正在清除 NuGet HTTP 快取: C:\Users\h1273\AppData\Local\NuGet\v3-cache
正在清除 NuGet 全域套件資料夾: D:\linqpad\nuget
正在清除 NuGet 暫存快取: C:\Users\h1273\AppData\Local\Temp\NuGetScratch
正在清除 NuGet 外掛程式快取: C:\Users\h1273\AppData\Local\NuGet\plugins-cache
已清除本機資源。
```
...
要從 csproj 引入 nuget 套件時，他會從當下的資料夾開始找有沒有 nuget.config 檔案
如果沒有，就繼續往上一層找，找到根目錄都沒有的話，就會已全域的設定為主


nuget.config 檔案必須加入專案版控，讓團隊的人共用
...
1. 人才問題：心血越來越不會去學 f ,要找到 f 的維護人才會越來越少
2. .net framerok 使用 C# 版本上限是 7.3，.net 使用 8.0 以上
# dotnet 的組成
...
<location path="." inheritInChildApplications="false">
  <system.webServer>
    ...
    <aspNetCore processPath="dotnet" arguments=".\w1.Application.dll" stdoutLogEnabled="false" stdoutLogFile=".\logs\stdout" hostingModel="inprocess" />
  </system.webServer>
</location>
</configuration>
```

如果 這個 config 有寫 `processPath="dotnet" arguments=".\w1.Application.dll"`  ，就代表不需要那個 exe 檔案了
但是如果沒有寫 arguments ，就需要這個 exe 檔案了
...
安裝的 .NET 工作負載:
[aspire]
  安裝來源: VS 17.10.35013.160
  資訊清單版本:    8.0.0/8.0.100
  資訊清單路徑:       C:\Program Files\dotnet\sdk-manifests\8.0.100\microsoft.net.sdk.aspire\8.0.0\WorkloadManifest.json
  安裝類型:        FileBased


Host:   # dotnet 的 runtime ，dotnet 的 runtime 是幫你跑的應用程式
Version:      8.0.6
Architecture: x64
Commit:       3b8b000a0e
...
sdk 跟 runtime 版本號，前兩碼會一樣，第三碼分開

一開始有介紹到，.net 可以開發很多種應用程式   console  桌面應用windowsform  網站  api 等
要啟動不同應用程式的 runtime 其實都不一樣，而且這些 runtime 都可以分開裝

而我們身為開發者，就只要 裝 sdk 全部就會裝在一起了
...
earing NuGet HTTP cache: C:\Users\h1273\AppData\Local\NuGet\v3-cache
Clearing NuGet global packages folder: D:\linqpad\nuget
Clearing NuGet Temp cache: C:\Users\h1273\AppData\Local\Temp\NuGetScratch
Clearing NuGet plugins cache: C:\Users\h1273\AppData\Local\NuGet\plugins-cache
